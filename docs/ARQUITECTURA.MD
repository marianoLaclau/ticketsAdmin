...
| `modules/rendimiento`    | Cohortes y agregaciones ejecutivas auditables: equipo, personas, reiteraciones y calidad de datos       |
...
La API valida entrada con schemas generados desde OpenAPI/Zod. Administración exige rol SysAdmin; Rendimiento exige la capacidad reservada a SysAdmin y Controller; y Controller accede a Tickets en modo de solo lectura. Estos controles se verifican en el backend en cada request, de forma independiente del frontend.
...
- `/rendimiento`: espacio ejecutivo visible únicamente para SysAdmin y Controller; comparte filtros y una cohorte visible entre equipo, personas, reiteraciones y calidad de datos.
...
define una cohorte de tickets visibles por `fecha_creacion` después de aplicar
...
```mermaid
flowchart LR
    filters[Filtros validados] --> cohort[Cohorte visible común]
    cohort --> team[Resumen del equipo]
    cohort --> people[Personas]
    cohort --> repeats[Reiteraciones]
...
Resumen y Personas distinguen la fotografía actual de los eventos históricos.
Una resolución solo es atribuible cuando un seguimiento prueba una transición
de estado no final a final y posee `autor_usuario_id`; el SLA histórico usa el
`fecha_limite_snapshot` preservado en ese evento. Las muestras y coberturas son
parte del contrato para no presentar estimaciones incompletas como hechos.

Reiteraciones asigna exactamente una clave canónica por ticket, con precedencia
DNI, teléfono y email. Una identidad secundaria sin DNI solo hereda uno cuando
la asociación directa en la cohorte es unívoca. No se calculan componentes
conexos ni se encadenan coincidencias, porque eso podría fusionar personas por
un dato compartido o incorrecto. Solo se publican grupos con dos tickets o más y
al menos uno abierto; el identificador completo nunca sale de la capa de datos,
la API entrega una máscara y un `grupo_id` opaco. Por tanto, el resultado es una
señal para revisión operativa y no una identidad civil inferida.
...
`autor_usuario_id` es una FK nullable a `usuarios.id` con `ON DELETE SET NULL`. Las acciones humanas nuevas guardan allí la identidad autenticada; los eventos automáticos y el historial anterior a la migración permanecen en `null`. La columna `autor` conserva el snapshot legible, por lo que eliminar o renombrar una cuenta no vuelve incomprensible la auditoría. El índice `(autor_usuario_id, fecha_creacion, id)` permite recuperar actividad atribuida con un orden estable.
...
Las API keys y el secreto bootstrap se entregan por variables de entorno o secretos del runner y no deben entrar al repositorio. `WEBHOOK_API_KEY` se valida al arrancar: debe existir, tener al menos 32 caracteres y no usar controles, espacios exteriores, un único carácter repetido ni placeholders públicos; los errores solo identifican la variable, nunca su valor. Las credenciales crudas no se persisten en la base: del secreto bootstrap solo se guarda un hash scrypt. Una credencial ya asegurada se conserva: la variable bootstrap no funciona como mecanismo de reset. La rotación de compatibilidad actualiza el hash, activa el cambio obligatorio y revoca las sesiones del seed histórico en una única transacción.
...
backend/                 API Express y composition root del servidor
  src/modules/           módulos funcionales con API pública propia
    ...
    rendimiento/         consultas, dominio y frontera HTTP de métricas ejecutivas
