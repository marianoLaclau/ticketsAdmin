...
- **Sesiones**: cookie host-only `httpOnly` + `SameSite=Lax`, con token aleatorio de 64 caracteres hexadecimales y respaldo en la tabla `sesiones` (revocables, sobreviven reinicios del backend), expiración absoluta a los 7 días. El backend elimina la cookie si está malformada, vencida, revocada o pertenece a una cuenta/rol inactivo; las respuestas `/auth/*` no se cachean. Al desactivar un usuario o rol se borran atómicamente sus sesiones y se cierran sus streams SSE, por lo que una reactivación posterior no revive cookies antiguas. El heartbeat SSE reutiliza el mismo extractor y revalida además la sesión cada 25 segundos. Las contraseñas usan scrypt asíncrono con parámetros/versionado explícitos; los hashes legados se rehashean al autenticarse y una identidad inexistente ejecuta el mismo KDF dummy antes de devolver el mismo `401` genérico. Login admite 1–128 caracteres por compatibilidad; no recorta ni aplica retroactivamente la política de altas.
