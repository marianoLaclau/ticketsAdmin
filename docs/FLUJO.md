...
| Ruta                                     | Qué hace                                                                                                                                                                                                          |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /api/webhooks/ticket`              | **Ingesta**: crea el ticket de una llamada. Única ruta autenticada con la API key de integración `x-api-key`. Idempotente. Si no viene `fecha_limite`, se preestablece a **48 horas hábiles de lunes a viernes**. |
...
| `GET /api/rendimiento/resumen-equipo`    | Conjunto analizado ingresado, estado actual, distribuciones, promedio/mediana de resolución y cumplimiento total del plazo sobre todas las finalizaciones con fechas verificables.                                |
| `GET /api/rendimiento/personas`          | Hechos operativos por usuario: finalizaciones, tiempos, cumplimiento total del plazo, carga actual y reaperturas. La API ordena alfabéticamente y no calcula puntajes ni rankings.                                |
| `GET /api/rendimiento/reiteraciones`     | Contactos con al menos dos tickets dentro del conjunto filtrado, aunque todos estén finalizados; agrupa por identidad canónica no transitiva, descarta valores centinela, enmascara PII y pagina el orden global. |
...
Rendimiento también agrega en SQLite y está disponible únicamente para **SysAdmin** y **Controller**. Sus vistas comparten un conjunto analizado de tickets visibles definido por `fecha_creacion` y por los filtros activos. El estado es una fotografía al instante informado por la respuesta y cada tiempo o porcentaje conserva su tamaño de muestra. Para completar la cobertura sin duplicar casos, Resumen del equipo y Operadores combinan transiciones de finalización que guardaron el plazo vigente con la última finalización reconstruible de los tickets que conservan `fecha_resolucion`, `fecha_limite` y responsable estructurado, pero no ese snapshot.

La API de Operadores devuelve hechos y los ordena alfabéticamente; no produce posiciones ni calificaciones. El frontend deriva un **índice descriptivo de rendimiento operativo**: 70 % corresponde al cumplimiento total del plazo y 30 % a la proporción de la carga abierta asignada que todavía no está vencida. Sin cierres con plazo medible no hay índice; con menos de cinco casos se presenta como **Muestra inicial** y permanece neutral. Desde cinco casos, 80 puntos o más se muestra como **Favorable** y un valor inferior como **Requiere atención**. El índice no cambia el orden alfabético ni constituye un ranking.

Contactos recurrentes devuelve coincidencias operativas enmascaradas, no identidades civiles inferidas; calcula sus totales sobre todos los grupos con dos tickets o más del conjunto analizado, sin exigir que conserven un caso abierto, y entrega únicamente la página solicitada después de aplicar el orden global. Los grupos totalmente finalizados informan cero abiertos y vencidos, antigüedad abierta y prioridad máxima `null`, y responsables actuales vacíos.

En **Resumen del equipo**, los cuatro indicadores se interpretan así:

- **Cumplimiento del plazo**: porcentaje de finalizaciones realizadas antes o exactamente en el plazo registrado. La muestra total combina transiciones reales de abierto a final que guardaron `fecha_limite_snapshot` con una única finalización reconstruible por ticket cuando su última resolución carece de snapshot, sin duplicar casos. La tarjeta presenta el total y usa la misma etiqueta elegida en el filtro, incluida **Período completo**. La parte reconstruida depende de `fecha_resolucion` y `fecha_limite` persistidas actualmente.
...
Los tres indicadores de backlog son una fotografía actual de los tickets **creados dentro del período seleccionado**. Elegir mes o semana no incluye abiertos más antiguos; para evaluar todo el backlog debe seleccionarse **Período completo**. Si no existen abiertos o finalizaciones con fechas suficientes, la interfaz muestra que no hay muestra disponible: no lo presenta como 0 %. Estos indicadores describen el estado observado y no implican por sí solos una meta, puntaje o evaluación de una persona.

El bloque **Tiempo de resolución** usa únicamente tickets actualmente `resuelto` o `cerrado` con `fecha_creacion` y `fecha_resolucion` coherentes. Mide horas corridas entre ambas fechas, por lo que incluye noches, fines de semana y feriados. La **mediana** es el valor central: la mitad de la muestra se resolvió en ese tiempo o menos y no se deforma tanto por unos pocos casos extremos. El **promedio** suma todas las duraciones y las divide por la cantidad de tickets; representa el costo temporal medio, pero aumenta cuando existen resoluciones excepcionalmente largas. Mostrar ambos permite distinguir el comportamiento habitual del impacto de esos casos demorados.
...
Las rutas `admin`, el borrado, la cuarentena y la edición técnica de tickets exigen sesión con rol SysAdmin. El backend lo valida en cada request de forma independiente del frontend: un Operador recibe `403` aunque manipule la interfaz.
...
Sigue visible en la tabla de Administración mediante `GET /api/tickets?incluir_vacios=true`. Ese parámetro no es un bypass público: exige sesión con rol SysAdmin.
...
- **Rendimiento** (`/rendimiento`): acceso exclusivo de SysAdmin y Controller. **Resumen del equipo** presenta tickets ingresados, abiertos, finalizados, vencidos, distribuciones, promedio/mediana y cumplimiento. **Operadores** muestra por nombre el volumen, los tiempos, el cumplimiento total, la carga actual y el índice descriptivo; se ven tres inicialmente y, al desplegar el resto, la lista queda contenida en un área de hasta 680 px con scroll. **Contactos recurrentes** incluye todos los contactos con al menos dos tickets dentro de los filtros, aunque sus casos estén totalmente finalizados, y muestra inicialmente tres tarjetas compactas con nombre, cantidad de llamados, abiertos, vencidos, prioridad abierta cuando corresponda, último llamado y responsable actual. Cada tarjeta permite desplegar la fecha del primer contacto, la antigüedad del abierto cuando corresponda, el reparto de responsables actuales y los tickets relacionados; al mostrar más contactos, la lista también queda contenida en un área de hasta 680 px con scroll. **Calidad de datos** permite interpretar la cobertura. El período puede ser **Período completo**, mes actual, semana actual, últimos 30 días, últimos 90 días o un rango personalizado. La vista activa y los filtros por período, empresa, categoría y prioridad se persisten y canonizan en la URL. Por eso un enlace directo abre el mismo contexto y la navegación Atrás/Adelante o una recarga restauran tanto la pestaña como sus filtros.
...
- **Roles y usuarios** (`/admin/roles-usuarios`): altas y edición de perfiles, asignación de rol, filtros, activación/desactivación y gestión del catálogo. `SysAdmin`, `Controller`, `Administrador` y `Operador` son roles base reservados: permanecen activos y no se renombran ni eliminan. Los roles personalizados inactivos cortan login/sesión y no aceptan nuevas asignaciones. Siempre debe quedar al menos un SysAdmin activo con username y contraseña utilizable. Todas las operaciones administrativas exigen una sesión SysAdmin y el backend vuelve a verificar el rol en cada request. Los campos de contraseña temporal permanecen ocultos, ofrecen un botón de ojo y validan la política compartida de 8–128 caracteres, sin controles, espacios exteriores ni valores predecibles conocidos. Alta y reset muestran el estado **Cambio de contraseña pendiente** y revocan accesos anteriores cuando corresponde.
...
El contrato declara el esquema `sessionCookie`, respaldado por la cookie `gsb_session`, como seguridad global. `healthz`, `readyz`, login y el logout idempotente son públicos; el webhook usa únicamente `x-api-key`. Las operaciones administrativas conservan la sesión como requisito HTTP y el backend aplica además la autorización SysAdmin en cada request. Las fronteras condicionales, como `incluir_vacios`, mantienen el acceso operativo normal y documentan la exigencia adicional al activar el modo administrativo. `healthz` conserva un contrato exclusivo `{ status: "ok" }`; `readyz` separa el éxito `{ status: "ready" }` del error 503 `{ status: "unavailable" }`, sin detalles técnicos y sin caché. Orval está fijado a la salida compatible con Zod 3.25 para no generar constructores exclusivos de Zod 4.
...
| `progreso`                            | entero 0-100           | Valor derivado del estado: `nuevo` 0, `en_proceso` 25, `pendiente` 50, `resuelto` 75 y `cerrado` 100. El cliente no lo elige de forma independiente.                                                                 |
...
| `fecha_limite`                        | timestamp              | **Plazo: 48 horas hábiles desde `fecha_creacion`**. Lunes a viernes cuentan las 24 h; sábado y domingo pausan el reloj. Es editable y una fecha explícita se respeta.                                                |
...
La máquina de estados es deliberadamente flexible porque una consulta puede resolverse en la misma llamada: no obliga a recorrer una secuencia lineal. `resuelto` y `cerrado` cuentan como estados finales para las métricas, pero un caso puede reabrirse hacia `en_proceso` o `pendiente`. Volver a `nuevo` está prohibido porque borraría la evidencia de que el ticket ya fue trabajado. En cada actualización el backend deriva `progreso` desde el estado y descarta cualquier intento de mantener ambos datos en contradicción.

### Plazo de resolución de 48 horas hábiles
...
**Cambios de schema**: en desarrollo local se editan los archivos de `lib/db/src/schema/` y se corre `pnpm --filter @workspace/db run push`. Ese script encadena `drizzle-kit push` con la reconciliación de invariantes que Drizzle no representa; debe ejecutarse sin otro escritor activo. Para que el cambio llegue al servidor hay que generar y commitear su migración SQL: el contenedor aplica la cadena versionada al arrancar. La secuencia actual llega a `0021_backfill_resolution_actors.sql`. Antes de escuchar requests, el backend verifica también la proyección de cuarentena; puede completar una base local legacy sin ledger, pero rechaza una base versionada incompleta.
...
- Preestablece `fecha_limite` a 48 horas hábiles desde `fecha_creacion` (la misma regla que usa el webhook), sin consumir plazo durante sábado ni domingo.
...
Esto es para **desarrollo local**. El servidor corre los mismos dos servicios en contenedores Docker y el workflow Deploy se dispara en cada push a `main`. Antes de modificar el servidor, Deploy reutiliza el workflow completo de Quality y espera tanto sus verificaciones generales como los E2E. El mismo workflow también valida pull requests — ver [docs/DEPLOY.md](DEPLOY.md) para el flujo real, el runbook y los backups.
...
- **Roles y capacidades**: `SysAdmin` tiene acceso total; `Controller` consulta Dashboard, Tickets y Rendimiento sin mutar tickets ni entrar en Administración; `Administrador` gestiona tickets y puede cerrarlos; `Operador` realiza la gestión básica, pero no puede cerrarlos. Los roles personalizados conservan la gestión básica. `SysAdmin`, `Controller`, `Administrador` y `Operador` son roles base protegidos, y el backend aplica estas capacidades aunque se manipule la interfaz.
- **Edición funcional**: los usuarios con capacidad de gestión pueden corregir nombre, apellido, teléfono, DNI/CUIT, empresa, email, motivo y resumen. Hora, notificación, audio y fechas límite/resolución son campos técnicos protegidos por el rol SysAdmin. La API vuelve a validar estos permisos aunque la UI oculte controles.
