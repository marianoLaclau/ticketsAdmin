...
Contactos recurrentes devuelve coincidencias operativas enmascaradas, no identidades civiles inferidas; calcula sus totales sobre todo el conjunto analizado y entrega únicamente la página solicitada después de aplicar el orden global por riesgo.
...
- **Rendimiento** (`/rendimiento`): acceso exclusivo de SysAdmin y Controller. **Resumen del equipo** presenta tickets ingresados, abiertos, finalizados, vencidos, distribuciones, promedio/mediana y cumplimiento. **Operadores** muestra por nombre el volumen, los tiempos, el cumplimiento total, la carga actual y el índice descriptivo; se ven tres inicialmente y, al desplegar el resto, la lista queda contenida en un área de hasta 680 px con scroll. **Contactos recurrentes** muestra inicialmente tres contactos en tarjetas compactas con nombre, cantidad de llamados, abiertos, vencidos, prioridad, último llamado y responsable actual. Cada tarjeta permite desplegar la fecha del primer contacto, la antigüedad del abierto, el reparto de responsables y los tickets relacionados; al mostrar más contactos, la lista también queda contenida en un área de hasta 680 px con scroll. **Calidad de datos** permite interpretar la cobertura. El período puede ser **Período completo**, mes actual, semana actual, últimos 30 días, últimos 90 días o un rango personalizado. La vista activa y los filtros por período, empresa, categoría y prioridad se persisten y canonizan en la URL. Por eso un enlace directo abre el mismo contexto y la navegación Atrás/Adelante o una recarga restauran tanto la pestaña como sus filtros.
