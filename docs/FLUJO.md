...
| `GET /api/rendimiento/resumen-equipo`    | Conjunto analizado ingresado, estado actual, distribuciones, promedio/mediana de resolución y cumplimiento total del plazo con desglose entre eventos auditables e historial reconstruido.                        |
| `GET /api/rendimiento/personas`          | Finalizaciones por usuario, separando eventos auditados de snapshots legacy sin evento; informa tiempos, cumplimiento auditable, carga y reaperturas, con cobertura global y sin rankings.                        |
...
Rendimiento también agrega en SQLite y está disponible únicamente para **SysAdmin** y **Controller**. Sus vistas comparten un conjunto analizado de tickets visibles definido por `fecha_creacion` y por los filtros activos. El estado es una fotografía al instante informado por la respuesta y los tiempos incluyen su tamaño de muestra. Operadores combina transiciones reales con tickets legacy finalizados que conservan fecha y responsable pero no un evento equivalente; estos últimos quedan identificados como históricos. La disponibilidad de comparación usa el conjunto combinado cuando su atribución es estructurada, mientras que el cumplimiento individual solo usa transiciones reales que conservaron `fecha_limite_snapshot`. El Resumen del equipo suma además una reconstrucción histórica explícita para la última finalización de cada ticket final sin snapshot, siempre que `fecha_resolucion` y `fecha_limite` sean coherentes y la resolución no sea futura respecto de `generado_en`; informa cumplidos y muestras de ambas fuentes y evita duplicar eventos auditados. Operadores nunca construye rankings. Contactos recurrentes devuelve coincidencias operativas enmascaradas, no identidades civiles inferidas; calcula sus totales sobre todo el conjunto analizado y entrega únicamente la página solicitada después de aplicar el orden global por riesgo.

En **Resumen del equipo**, los cuatro indicadores se interpretan así:

- **Cumplimiento del plazo**: porcentaje de finalizaciones realizadas antes o exactamente en el plazo registrado. La muestra total combina transiciones reales de abierto a final que guardaron `fecha_limite_snapshot` con una única finalización histórica reconstruible por ticket cuando su última resolución carece de snapshot. La tarjeta muestra cuánto proviene de cada fuente y usa la misma etiqueta elegida en el filtro, incluida **Todo el historial**. La parte reconstruida depende de `fecha_resolucion` y `fecha_limite` persistidas actualmente; no tiene la misma fuerza probatoria que un evento auditable.
...
Los tres indicadores de backlog son una fotografía actual de los tickets **creados dentro del período seleccionado**. Elegir mes o semana no incluye abiertos más antiguos; para evaluar todo el backlog debe usarse todo el historial. Si no existen abiertos o finalizaciones con fechas suficientes, la interfaz muestra que no hay muestra disponible: no lo presenta como 0 %. Estos indicadores describen el estado observado y no implican por sí solos una meta, puntaje o evaluación de una persona.
