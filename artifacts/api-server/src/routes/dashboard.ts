...
import { eq, lt, not, inArray, gte, lte, count, avg, sql } from "drizzle-orm";
...
router.get("/dashboard/stats", async (req, res) => {
  ...
  const [allTickets, vencidosResult, resueltosHoy, nuevosHoy] = await Promise.all([
    ...
    db.select({ id: ticketsTable.id }).from(ticketsTable).where(
      sql`${ticketsTable.fecha_limite} < ${now} AND ${ticketsTable.estado} NOT IN ('resuelto', 'cerrado')`
    ),
    db.select({ id: ticketsTable.id }).from(ticketsTable).where(
      sql`${ticketsTable.fecha_resolucion} >= ${todayStart} AND ${ticketsTable.fecha_resolucion} < ${todayEnd}`
    ),
    db.select({ id: ticketsTable.id }).from(ticketsTable).where(
      sql`${ticketsTable.fecha_creacion} >= ${todayStart} AND ${ticketsTable.fecha_creacion} < ${todayEnd}`
    ),
  ]);
...
router.get("/dashboard/actividad-reciente", async (req, res) => {
  ...
  const recentTickets = await db.select().from(ticketsTable).orderBy(sql`${ticketsTable.fecha_creacion} DESC`).limit(limit);
  const recentSeguimientos = await db.select({ seg: seguimientosTable, ticket: { nombre: ticketsTable.nombre, apellido: ticketsTable.apellido } })
    ...
    .orderBy(sql`${seguimientosTable.fecha_creacion} DESC`)
    .limit(limit);
...
router.get("/dashboard/tickets-vencidos", async (req, res) => {
  ...
  const tickets = await db.select().from(ticketsTable).where(
    sql`${ticketsTable.fecha_limite} < ${now} AND ${ticketsTable.estado} NOT IN ('resuelto', 'cerrado')`
  ).orderBy(ticketsTable.fecha_limite).limit(20);
  res.json(tickets);
});
