...
La lista muestra inicialmente tres operadores y permite desplegar o contraer el resto; al ampliarse queda contenida en un área con `max-height: 680px` y scroll propio. **Contactos recurrentes** presenta todos los contactos con dos o más tickets dentro de los filtros, incluso cuando ya no tienen gestiones abiertas. Muestra primero tres contactos con un resumen compacto: nombre de referencia, coincidencia enmascarada, cantidad de llamados, abiertos, vencidos, prioridad máxima cuando existe, último llamado y situación actual. Los grupos totalmente finalizados llevan la marca **Finalizado · 0 abiertos** y no se presentan engañosamente como casos sin asignar; su detalle aclara que no quedan tickets abiertos. **Ver detalles** despliega la fecha del primer contacto, la antigüedad del abierto cuando corresponde, responsables actuales y tickets relacionados; dentro de cada contacto se muestran inicialmente tres tickets y se puede abrir el resto. Al desplegar más contactos, la lista también usa un máximo de 680 px con scroll, sin perder la paginación del servidor. La ruta usa `RendimientoRouteGuard`, y el sidebar solo ofrece el acceso cuando `puedeVerRendimiento(me?.rol)` es verdadero.
...
- Reproductor `<audio>` nativo si el ticket tiene `audio_url`. El frontend reproduce la referencia recibida sin decidir su proveedor: normalmente corresponde al storage local corporativo y puede venir de OneDrive cuando n8n utilizó el fallback.
