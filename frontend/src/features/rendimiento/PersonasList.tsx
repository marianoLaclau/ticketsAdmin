...
import type { RendimientoPersona } from "@workspace/api-client-react";
import {
  ChevronDown,
  ChevronUp,
  Gauge,
    ...
    <div className="min-w-0 rounded-lg border border-slate-200 bg-slate-50/70 p-3">
      ...
      {performance.state === "muestra_inicial" ? (
        <dd className="mt-0.5 text-[10px] leading-relaxed text-muted-foreground">
          ...
          {rendimientoNumberFormatter.format(performance.deadlineSample)} de{" "}
      ...
      >
        ...
        <dl className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          <PersonFact
            ...
            value={rendimientoNumberFormatter.format(person.tickets_resueltos)}
            details={[
              pluralize(
          ...
          <PersonFact
            ...
            details={
              compliance.muestra > 0
                ? [
                    rendimientoNumberFormatter.format(compliance.cumplidos) +
                      " de " +
