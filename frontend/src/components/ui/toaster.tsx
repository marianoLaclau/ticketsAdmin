...
export function Toaster() {
  ...
  return (
    <ToastProvider label="Notificación">
      {toasts.map(function ({
        id,
        title,
        description,
        ...
        return (
          <Toast
            key={id}
            variant={variant}
            duration={duration ?? visual.duration}
            {...props}
          >
            <div
              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${visual.iconClassName}`}
            >
      ...
      <ToastViewport label="Notificaciones ({hotkey})" />
    </ToastProvider>
  );
}
