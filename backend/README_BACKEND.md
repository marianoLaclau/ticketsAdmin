...
- Desactivar un usuario o un rol desde Administración elimina en la misma transacción todas las sesiones afectadas y cierra sus conexiones SSE abiertas. Cambiar realmente `role_id` hace lo mismo, de modo que una cookie previa no hereda una promoción ni conserva permisos obsoletos; reenviar el mismo rol no expulsa al usuario. Reactivar una cuenta o rol no revive cookies anteriores. El heartbeat del stream vuelve a validar la cookie cada 25 segundos como defensa adicional ante expiraciones o revocaciones externas.
