...
### Rendimiento: alcance auditable actual
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Cumplimiento auditable del plazo | `cumplidos / muestra`, donde la muestra contiene exclusivamente seguimientos que prueban una transición de estado no final a `resuelto` o `cerrado` y conservaron `fecha_limite_snapshot`. Cumple si `fecha_creacion` del evento es menor o igual al snapshot del plazo. Una transición posterior de `resuelto` a `cerrado` no agrega otra resolución. |
...
Los tres indicadores de backlog describen el estado actual de los tickets creados dentro del conjunto analizado: no incorporan tickets creados fuera del período solicitado. Con cero abiertos, sus porcentajes y la mediana son `null`; el cumplimiento del plazo también devuelve `porcentaje: null` cuando no existe una transición con snapshot auditable. Esto significa **sin muestra**, no 0 %. Las resoluciones anteriores a la incorporación de `fecha_limite_snapshot` permanecen fuera de esa medición y no se reconstruyen a partir del plazo actual. Los plazos persistidos se toman como autoritativos; no se recalculan retroactivamente los registros históricos creados con reglas anteriores.

**Personas** atribuye cada resolución exclusivamente al `autor_usuario_id` de ese evento, no al responsable actual del ticket; informa por separado su muestra y conserva usuarios inactivos para no borrar historia.
