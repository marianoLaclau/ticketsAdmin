...
| `GET /rendimiento/resumen-equipo`   | Resumen real del mismo conjunto analizado: tickets ingresados, fotografía de estados y prioridades, abiertos/finalizados/vencidos, promedio y mediana de resolución con muestra, y cumplimiento del plazo con muestra auditable.                                                                                                                                                                                                                                                                                                                                                                                                   | ✅🧭                             |
...
El cumplimiento del plazo es más estricto: solo considera seguimientos que representan una transición real desde un estado no final hacia `resuelto` o `cerrado` y que conservaron `fecha_limite_snapshot` al resolverse. El paso posterior de `resuelto` a `cerrado` no duplica una resolución. **Personas** atribuye cada resolución exclusivamente al `autor_usuario_id` de ese evento, no al responsable actual del ticket; informa por separado su muestra y conserva usuarios inactivos para no borrar historia.
