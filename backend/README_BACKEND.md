...
| `GET /rendimiento/personas`         | Indicadores descriptivos por usuario: tickets y eventos de resolución atribuibles, promedio/mediana con muestra, cumplimiento con snapshot del plazo, carga abierta actual y resoluciones luego reabiertas. Incluye cobertura global y nunca produce puntajes ni rankings.                                                                                                                                                                                                                                                                                                                                                         | ✅🧭                             |
...
**Personas** usa el `autor_usuario_id` de cada resolución real. La migración `0021` completa ese ID en resoluciones legacy solo cuando la última asignación auditada hasta el evento y el snapshot textual del autor corroboran la misma identidad. Además incorpora como `finalizaciones_historicas` los tickets finales con `fecha_resolucion` y responsable estructurado que no poseen una transición equivalente; sirven para cantidad y tiempo, pero no para cumplimiento del plazo ni reaperturas. La respuesta expone ambas fuentes por separado y conserva usuarios inactivos para no borrar historia.

El porcentaje de atribución y `comparacion_individual_estado` describen todos los hechos de finalización visibles: transiciones reales y finalizaciones históricas atribuibles. La comparación exige al menos diez hechos y aplica sus umbrales sobre la atribución estructurada del conjunto combinado. Cada fuente permanece separada en la respuesta; el cumplimiento individual del plazo continúa siendo estrictamente auditable y solo usa transiciones reales con `fecha_limite_snapshot`.
