...
El cumplimiento del plazo es más estricto: solo considera seguimientos que representan una transición real desde un estado no final hacia `resuelto` o `cerrado` y que conservaron `fecha_limite_snapshot` al resolverse. El paso posterior de `resuelto` a `cerrado` no duplica una resolución. Las vistas **Personas** y **Reiteraciones** siguen en preparación; hasta disponer de cobertura suficiente no se publican rankings ni se atribuyen resultados al responsable actual del ticket.
