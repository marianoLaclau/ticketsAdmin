...
backend/
  ...
  src/
    ...
    modules/
      ...
      rendimiento/         → Calidad y Resumen ejecutivo, con consultas SQL y frontera protegida
...
| `GET /dashboard/stats`              | Totales por estado/prioridad, vencidos, resueltos hoy/período, nuevos hoy/período y tiempo promedio. Admite `fecha_desde`/`fecha_hasta` inclusivas por fecha de creación; los resueltos del período pertenecen a ese mismo conjunto analizado.                                                                                                                                                                                                                                                                                                                                                                                     | ✅                               |
...
| `GET /rendimiento`                  | Estado contract-first del módulo ejecutivo: devuelve `estado: "operativo"` y sus cuatro vistas. Toda la frontera `/rendimiento` está protegida por `requirePerformanceAccess` y responde con `Cache-Control: private, no-store`.                                                                                                                                                                                                                                                                                                                                                                                                   | ✅🧭                             |
| `GET /rendimiento/calidad-datos`    | Cobertura auditable de autoría, fechas de resolución, snapshot del plazo, asignación estructurada, identidad de contacto y fecha límite. Usa un único conjunto analizado visible por `fecha_creacion` y los filtros opcionales de período, empresa, categoría y prioridad.                                                                                                                                                                                                                                                                                                                                                         | ✅🧭                             |
| `GET /rendimiento/resumen-equipo`   | Resumen real del mismo conjunto analizado: tickets ingresados, fotografía de estados y prioridades, abiertos/finalizados, promedio/mediana de resolución y cuatro indicadores con base explícita: cumplimiento total del plazo con desglose auditable/histórico, backlog vencido, antigüedad mediana hábil y cobertura de asignación.                                                                                                                                                                                                                                                                                              | ✅🧭                             |
| `GET /rendimiento/personas`         | Hechos descriptivos por usuario: tickets y resoluciones atribuibles, promedio/mediana con muestra, cumplimiento total y estricto del plazo, carga abierta actual y resoluciones luego reabiertas. Incluye cobertura global y los insumos que el frontend usa para su índice operativo; el backend no calcula puntajes, estados de rendimiento, posiciones ni rankings.                                                                                                                                                                                                                                                             | ✅🧭                             |
| `GET /rendimiento/reiteraciones`    | Contactos con al menos dos tickets del conjunto analizado, incluso si todos están finalizados. Usa una clave canónica única DNI → teléfono → email, solo hereda relaciones directas unívocas, evita uniones transitivas, enmascara PII y ordena primero los grupos que conservan gestiones abiertas.                                                                                                                                                                                                                                                                                                                               | ✅🧭                             |
...
### Rendimiento: alcance trazable actual

Los endpoints operativos de Rendimiento aceptan los mismos filtros opcionales (`fecha_desde`, `fecha_hasta`, `empresa`, `motivo_categoria` y `prioridad`), rechazan valores repetidos y normalizan los días calendario con `America/Argentina/Buenos_Aires`. Solo **SysAdmin** y **Controller** pueden consultarlos; las respuestas no se cachean.

El conjunto analizado contiene tickets operativos —sin cuarentena— según su `fecha_creacion`. El bloque `estado_actual` es una fotografía de ese conjunto analizado al instante `generado_en`, no una reconstrucción histórica. Los tiempos de resolución informan muestra, promedio y mediana únicamente para tickets actualmente finalizados con fechas válidas y duración no negativa.

El resumen expone cuatro indicadores con su base de cálculo explícita:

| Indicador                      | Fórmula y alcance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Cumplimiento total del plazo   | `cumplidos / muestra`. Incluye cada transición real no-final → final que conservó `fecha_limite_snapshot`. Para un ticket actualmente final sin snapshot en su última resolución —o sin evento equivalente— incorpora una sola reconstrucción mediante sus `fecha_resolucion` y `fecha_limite` persistidas, coherentes y no futuras respecto de `generado_en`. Una finalización con snapshot nunca se duplica. `cumplimiento_plazo_auditable` conserva por separado la medición estricta y `cumplimiento_plazo` informa el total más el desglose de ambas fuentes. |
| Backlog vencido                | `vencidos / abiertos`. Abierto significa estado actual distinto de `resuelto` y `cerrado`; vencido exige además `fecha_limite < generado_en`. `con_plazo` permite conocer cuántos abiertos tienen un vencimiento verificable. Un límite exactamente igual al snapshot todavía no está vencido.                                                                                                                                                                                                                                                                     |
| Antigüedad mediana del backlog | Mediana exacta de `calcularHorasHabilesEntre(fecha_creacion, generado_en)` sobre abiertos cuya creación no está en el futuro. Cuenta las 24 horas de lunes a viernes en `America/Argentina/Buenos_Aires`, pausa sábado y domingo y no excluye feriados.                                                                                                                                                                                                                                                                                                            |
| Cobertura de asignación        | `asignados / abiertos`. Solo `asignado_usuario_id` identifica una asignación operativa; `asignado_a` es un snapshot textual histórico y no suma por sí solo. `sin_asignar = abiertos - asignados`.                                                                                                                                                                                                                                                                                                                                                                 |

Los tres indicadores de backlog describen el estado actual de los tickets creados dentro del conjunto analizado: no incorporan tickets creados fuera del período solicitado. Con cero abiertos, sus porcentajes y la mediana son `null`; el cumplimiento total también devuelve `porcentaje: null` cuando ninguna finalización posee fechas suficientes y coherentes. Esto significa **sin muestra**, no 0 %. La fuente histórica es una reconstrucción explícita con los valores actualmente persistidos: puede cambiar si se editan esas fechas y no sustituye la evidencia inmutable de `fecha_limite_snapshot`. Los plazos guardados se respetan tal como están; no se recalculan retroactivamente con reglas de calendario posteriores.

**Personas** usa el `autor_usuario_id` de cada resolución real. La migración `0021` completa ese ID en resoluciones legacy solo cuando la última asignación auditada hasta el evento y el snapshot textual del autor corroboran la misma identidad. Además incorpora como `finalizaciones_historicas` los tickets finales con `fecha_resolucion` y responsable estructurado que no poseen una transición equivalente; sirven para cantidad y tiempo. Para el cumplimiento total, una única finalización actualmente persistida puede reconstruirse con `fecha_resolucion` y `fecha_limite` cuando su último evento no preservó el snapshot; `cumplimiento_plazo_auditable` conserva en paralelo la medición estricta. La respuesta expone las fuentes por separado y conserva usuarios inactivos para no borrar historia.

El porcentaje de atribución y `comparacion_individual_estado` describen todos los hechos de finalización disponibles: transiciones reales y finalizaciones históricas atribuibles. La comparación exige al menos diez hechos y aplica sus umbrales sobre la atribución estructurada del conjunto combinado. Cada fuente permanece separada en la respuesta. `cumplimiento_plazo` es la medición total que consume la vista de Operadores y `cumplimiento_plazo_auditable` queda disponible como evidencia estricta.

El índice **Rendimiento operativo** no pertenece al dominio del backend ni al contrato: el frontend lo deriva con 70 % de `cumplimiento_plazo` y 30 % de la proporción de carga abierta asignada que aún no venció. La presentación es favorable desde 80 puntos y permanece neutral como muestra inicial mientras haya menos de cinco cierres con plazo medible. Si no existe ninguna muestra de cumplimiento no calcula el índice. Es una señal descriptiva por operador, no un ranking. Aunque el contrato conserva `resoluciones_reabiertas` como contexto técnico, la interfaz actual no muestra una tarjeta de reaperturas ni incorpora ese dato al índice.

**Reiteraciones** agrupa dentro de ese mismo conjunto analizado y nunca por nombre. Cada ticket recibe una única clave con precedencia DNI, teléfono y email. Un teléfono o email sin DNI solo hereda uno cuando apunta directamente a un único DNI; las relaciones ambiguas quedan separadas y no se encadenan como componentes conexas. Se incluyen todos los grupos con dos tickets o más que cumplen los filtros, aunque tengan `abiertos: 0`. En un grupo totalmente finalizado, `vencidos_abiertos` vale cero, `antiguedad_abierto_horas` y `prioridad_maxima` son `null`, y `responsables` es `[]`; cuando existen abiertos, la prioridad máxima se calcula únicamente sobre ellos. La respuesta no devuelve DNI, teléfono o correo completos: expone un valor enmascarado, cobertura, ambigüedades y links a los tickets para revisión humana. Es una señal operativa, no una identificación civil ni una afirmación de que el contacto no recibió respuesta.
