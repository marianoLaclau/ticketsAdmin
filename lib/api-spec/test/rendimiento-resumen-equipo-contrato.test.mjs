...
describe("contrato del resumen de equipo de Rendimiento", () => {
  ...
  it("expone el cumplimiento histórico total sin ocultar sus fuentes", () => {
    const compliance = schema("RendimientoCumplimientoPlazo");
    ...
    assert.deepEqual(compliance.required, [
      ...
      "porcentaje",
      "muestra_auditable",
      "cumplidos_auditables",
      "muestra_historica_reconstruida",
      "cumplidos_historicos_reconstruidos",
    ]);
    for (const property of [
      ...
      assert.equal(compliance.properties[property].type, "integer");
      assert.equal(compliance.properties[property].minimum, 0);
    }
    assert.deepEqual(compliance.properties.porcentaje.type, ["number", "null"]);
    assert.equal(compliance.properties.porcentaje.minimum, 0);
    assert.equal(compliance.properties.porcentaje.maximum, 100);
    assert.match(compliance.description, /fecha_limite_snapshot/);
    assert.match(compliance.description, /fecha_resolucion.*fecha_limite/is);
    assert.match(compliance.description, /hasta generado_en/i);
    assert.match(compliance.description, /distinguir evidencia auditable/is);
  });
  ...
  it("cierra la respuesta y conserva el período con timezone y snapshot", () => {
    ...
    assert.equal(
      response.properties.cumplimiento_plazo.$ref,
      "#/components/schemas/RendimientoCumplimientoPlazo",
    );
    assert.equal(
