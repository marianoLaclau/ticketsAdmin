...
describe("contrato de Rendimiento individual", () => {
  ...
  it("fija atribución, tiempos, carga y reaperturas sin convertirlos en ranking", () => {
    ...
    assert.match(description, /no representa\s+tiempo exclusivo/i);
  ...
  it("expone cobertura global estricta y los umbrales reales de comparación", () => {
    ...
    assert.deepEqual(coverage.required, [
      ...
      "finalizaciones_historicas_detectadas",
      "finalizaciones_historicas_atribuidas",
    ...
    assert.match(
      coverage.properties.comparacion_individual_estado.description,
      /todos los hechos.*transiciones reales.*históric/is,
    );
  ...
  it("distingue tickets resueltos de eventos de resolución atribuibles", () => {
    ...
    assert.match(
      person.properties.finalizaciones_historicas_atribuidas.description,
      /tickets legacy.*sin evento/is,
    );
    assert.match(
    ...
    assert.equal(
      person.properties.cumplimiento_plazo.$ref,
      "#/components/schemas/RendimientoCumplimientoPersona",
    );
  });
  ...
  it("mantiene explícitas las muestras nulas, el SLA y la carga actual", () => {
    ...
    const combinedCompliance = schema("RendimientoCumplimientoPersona");
    ...
    assert.equal(combinedCompliance.additionalProperties, false);
    assert.deepEqual(combinedCompliance.required, [
      "muestra",
      "cumplidos",
      "porcentaje",
    ]);
    assertNonNegativeIntegerProperties(combinedCompliance, [
      "muestra",
      "cumplidos",
    ]);
    assert.deepEqual(combinedCompliance.properties.porcentaje.type, [
      "number",
      "null",
    ]);
    assert.equal(combinedCompliance.properties.porcentaje.minimum, 0);
    assert.equal(combinedCompliance.properties.porcentaje.maximum, 100);
    assert.match(
      combinedCompliance.description,
      /Combina eventos.*única.*última finalización/is,
    );
    assert.match(
      combinedCompliance.description,
      /porcentaje es null cuando muestra es cero/i,
    );
