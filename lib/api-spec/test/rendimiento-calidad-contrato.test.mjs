...
describe("contrato de calidad de datos de Rendimiento", () => {
  it("declara el módulo operativo con sus cuatro vistas", () => {
    const status = schema("RendimientoModuleStatus");

    assert.deepEqual(status.properties.estado.enum, ["operativo"]);
    assert.deepEqual(status.properties.vistas.items.enum, [
      "resumen_equipo",
      "personas",
      "reiteraciones",
      "calidad_datos",
  ...
  it("publica únicamente una consulta autenticada con un conjunto analizado auditable", () => {
  ...
  it("mantiene estrictos el período, las proporciones y las coberturas", () => {
    ...
    for (const name of expectedCoverageNames) {
      ...
      assert.match(
        coverages.properties[name].description,
        /conjunto analizado/i,
      );
    }
  });
