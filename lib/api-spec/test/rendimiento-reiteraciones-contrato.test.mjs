...
describe("contrato de reiteraciones de contactos", () => {
  ...
  it("define una identidad canónica no transitiva y limitada al conjunto analizado", () => {
    ...
    assert.match(description, /al menos dos.*tickets\s+distintos/is);
    assert.match(description, /incluso si todos.*resueltos o cerrados/is);
  ...
  it("pagina después del orden global y conserva el resumen del conjunto analizado", () => {
    ...
    assert.match(
      ...
      /orden.*conjunto completo.*antes de\s+paginar/is,
    );
  ...
  it("admite grupos finalizados y explicita sus indicadores operativos", () => {
    ...
    assert.equal(contact.properties.abiertos.minimum, 0);
    assert.match(
      contact.properties.abiertos.description,
      /cero.*todos.*finalizados/i,
    );
    assert.match(
      contact.properties.vencidos_abiertos.description,
      /cero.*no hay abiertos/i,
    );
    ...
    assert.match(
      contact.properties.antiguedad_abierto_horas.description,
      /null.*totalmente finalizado/i,
    );
    assert.deepEqual(contact.properties.prioridad_maxima.type, [
      "string",
      "null",
    ]);
    ...
    assert.match(
      contact.properties.prioridad_maxima.description,
      /tickets abiertos.*null.*totalmente finalizado/is,
    );
    assert.match(
      contact.properties.responsables.description,
      /arreglo vacío.*totalmente finalizado/i,
    );
    ...
    assert.match(
      contract.paths["/rendimiento/reiteraciones"].get.description,
      /tickets abiertos.*antes.*completamente\s+finalizados.*vencimiento.*prioridad máxima.*antigüedad.*último\s+contacto/is,
    );
  });
