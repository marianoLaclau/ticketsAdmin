...
const canonicalComposeContract = JSON.stringify({
  contract: "ticketsadmin-compose-e2e-v1",
});
...
type FakeState = {
  ...
  phase: "baseline" | "candidate-active" | "candidate-partial" | "rolled-back";
  volumeId: string;
};
...
if (args[0] === "compose" && args[1] === "config") {
  output(JSON.stringify(composeConfig));
  process.exit(0);
}
...
if (args[0] === "image" && args[1] === "inspect") {
  ...
  if (format === "{{.Id}}") output(identity);
    ...
    output(baseline ? (preLedgerBaseline ? preLedgerBaselineRelease : "baseline-release") : releaseId);
  ...
  const result = spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        FAKE_RELEASE_ID: `git-${revision}-run-321-1`,
