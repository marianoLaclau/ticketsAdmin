...
- **Trazabilidad desde v0.5**: cada modificación registra de forma atómica el autor y los cambios reales de estado, prioridad, asignación y campos editados. Una migración recupera autores de resoluciones anteriores solo cuando la asignación estructurada y el snapshot del nombre coinciden; nunca fabrica eventos faltantes.
...
- **Rendimiento ejecutivo trazable**: SysAdmin y Controller disponen de `/rendimiento` con sus cuatro vistas operativas y un período **Todo el historial**. **Resumen del equipo** muestra el conjunto analizado visible creado en el período, estado, distribuciones, tiempos y cuatro indicadores explícitos: cumplimiento total del plazo, porcentaje del backlog vencido, antigüedad mediana del backlog en horas hábiles y cobertura de asignación estructurada. El cumplimiento combina eventos que preservaron el vencimiento con la última finalización histórica reconstruible a partir de `fecha_resolucion` y `fecha_limite`, e informa ambas fuentes por separado para no presentar datos legacy como auditoría inmutable. Cada indicador conserva su muestra o denominador y queda sin porcentaje/mediana cuando no existe una base válida; no se inventan fechas ausentes ni metas de Dirección. **Operadores** combina resoluciones auditadas con finalizaciones legacy identificadas como tales para recuperar volumen, tiempos y cobertura comparativa de todo el historial atribuible, sin mezclarlas en su cumplimiento individual auditable; muestra tres personas inicialmente y permite desplegar el resto, siempre sin rankings. **Contactos recurrentes** detecta y pagina por riesgo contactos con múltiples tickets y al menos uno abierto mediante una identidad canónica no transitiva y siempre enmascarada; **Calidad de datos** explicita la cobertura auditable de cada medición.
...
| `N8N_CHAT_WEBHOOK_URL`             | URL HTTPS privada del Chat Trigger que atiende al asistente de Rendimiento                                                                                |
| `N8N_CHAT_BASIC_AUTH_USER`         | Usuario Basic Auth que el backend agrega al request saliente del asistente                                                                                |
| `N8N_CHAT_BASIC_AUTH_PASSWORD`     | Contraseña Basic Auth del Chat Trigger; nunca se entrega al navegador                                                                                     |
| `N8N_CHAT_TIMEOUT_MS`              | Timeout del workflow del asistente en milisegundos (default y máximo 120000; mínimo 1000)                                                                 |
