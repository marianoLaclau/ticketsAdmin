...
- **Login real con roles**, no solo una API key: sesiones en cookie `httpOnly` respaldadas en tabla, contraseñas con scrypt asíncrono y formato versionado, limitación por identidad más admisión criptográfica acotada, y un candado global (`requireSession`) que protege toda la API salvo el webhook y el propio login.
