...
- **Rendimiento ejecutivo auditable**: SysAdmin y Controller disponen de `/rendimiento` con sus cuatro vistas operativas. **Resumen del equipo** muestra la cohorte visible creada en el período, estado, distribuciones, tiempos y cumplimiento del plazo; **Operadores** presenta actividad atribuible, muestras, carga y reaperturas sin rankings; **Contactos recurrentes** detecta y pagina por riesgo contactos con múltiples tickets y al menos uno abierto mediante una identidad canónica no transitiva y siempre enmascarada; **Calidad de datos** explicita la cobertura de cada medición.
